...
function show_options() {
    ...
    function show_timetable_options() {
        ...
        for (day_object of config.data.timetable) {
            ...
            for (var i = 0; i < config.data.periods.length; i++) {
                try {
                    ...
                    add_period_input_group(day_object.schedule[i].subject, day_object.schedule[i].room,day_object.schedule[i].availability);
                } catch (TypeError) {
        ...
        function add_period_input_group(subject, room,room_available) {
            schedule_inputs_container.innerHTML +=
                "<div class='input_group'>" +
                `<input value='${subject}' placeholder="Subject"/>` +
                `<input value='${room}' placeholder="Room"/>` +
                `<input value='${room_available} placeholder="Availability"/>`+
                "</div>";
        }
    }
}
...
function save() {
    ...
    for (var i = 0; i < day_name_inputs.length; i++) {
        ...
        for (input_group of schedule_input_groups) {
            ...
            var room_available=input_group.children[2].value;
